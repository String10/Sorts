...
public:
    TimSort();
    ~TimSort();
    static void sort(T [], size_t, Comparer=def_cmp);
private:
    T temp, *arr_helper;
    size_t max_size;
    static bool def_cmp(const T &, const T &);
    static void swap(T &, T&);
    static size_t minRunLength(size_t);
    static size_t upperbound(const T [], const T &, size_t, size_t, Comparer);
    static void insertSort(T [], size_t, Comparer);
};
...
template <typename T>
size_t TimSort<T>::minRunLength(size_t len) {
    size_t sticky = 0;
    while(len >= (1 << 6)) {
        sticky |= (len & 1);
        len >>= 1;
...
template <typename T>
size_t TimSort<T>::upperbound(const T arr[], const T &val, size_t l, size_t r, Comparer cmp) {
    size_t mid;
    while(l < r) {
        mid = (l + r) / 2;
        if(cmp(val, arr[mid])) {
